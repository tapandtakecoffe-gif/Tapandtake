...
import { ArrowLeft, LogOut, Clock, MoreVertical, Search, CheckCircle2, XCircle, User, UserCircle2, TrendingUp, Timer, Calendar as CalendarIcon } from 'lucide-react';
...
const Admin = () => {
  ...
  const filteredAndSortedOrders = useMemo(() => {
    ...
    return filtered;
  }, [dateFilteredOrders, searchQuery, statusFilter, sortBy]);
  ...
  // Calculate average preparation time for selected date
  const averagePrepTime = useMemo(() => {
    const completedOrdersWithTime = dateFilteredOrders.filter(o => 
      o.status === 'completed' && 
      o.estimatedPickupTime && 
      o.timestamp
    );
    
    if (completedOrdersWithTime.length === 0) return null;
    
    const totalMinutes = completedOrdersWithTime.reduce((sum, order) => {
      const start = new Date(order.timestamp).getTime();
      const end = new Date(order.estimatedPickupTime!).getTime();
      return sum + (end - start) / (1000 * 60);
    }, 0);
    
    return Math.round(totalMinutes / completedOrdersWithTime.length);
  }, [dateFilteredOrders]);
  ...
  const handleTimeChange = (orderId: string, minutes: number) => {
    const now = new Date();
    const estimatedTime = new Date(now.getTime() + minutes * 60000);
    updateEstimatedPickupTime(orderId, estimatedTime);
    toast({
      title: "Time updated",
      description: `Estimated pickup time set to ${minutes} minutes`,
    });
  };
  ...
  // Generate sample orders for the last 7 days
  const handleGenerateSampleOrders = () => {
    if (!confirm('This will add sample orders to your existing orders. Continue?')) {
      return;
    }
    ...
    // Get only non-addon products
    const availableProducts = products.filter(p => !p.isAddOn);

    // Configuration for orders per day
    const ordersConfig = [
      { day: 0, count: 3 },  // Today
      { day: 1, count: 5 },  // Yesterday
      { day: 2, count: 4 },  // 2 days ago
      { day: 3, count: 6 },  // 3 days ago
      { day: 4, count: 3 },  // 4 days ago
      { day: 5, count: 7 },  // 5 days ago
      { day: 6, count: 4 },  // 6 days ago
    ];

    // Get existing order IDs from current orders to avoid duplicates
    ...
    ordersConfig.forEach(({ day, count }) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (day === 0) {
          // Today: some active, some completed
          ...
          // Past days: mostly completed, some cancelled
          status = Math.random() > 0.9 ? 'cancelled' : 'completed';
        }

        // Estimated pickup time (only for completed or ready)
        ...
        while (existingOrderIds.has(orderId) || sampleOrders.some(o => o.id === orderId)) {
          orderNumber++;
          orderId = `DEMO${String(day + 1).padStart(2, '0')}${String(orderNumber).padStart(3, '0')}`;
        }
    ...
    // Add all sample orders to existing orders
    ...
    toast({
      ...
      description: `Added ${sampleOrders.length} sample orders for the last 7 days`,
    });
  ...
  const CountdownTimer = ({ targetTime }: { targetTime: Date }) => {
    ...
    const isUrgent = minutes < 5 && timeLeft > 0;
    ...
    return (
      <span className={`text-xs font-semibold ${isUrgent ? 'text-red-600' : 'text-primary'}`}>
  ...
  const OrderCard = ({ order }: { order: Order }) => {
    ...
    const handleTimeSubmit = (e: React.FormEvent) => {
      ...
      if (minutes > 0) {
        handleTimeChange(order.id, minutes);
        setTimeInput('');
      }
    };

    const isRegisteredUser = !!order.userId;
        ...
        <CardContent className="space-y-4">
          ...
          {isActive && (
            <div className="space-y-3">
              ...
              {/* Time Estimator with Quick Buttons */}
              <div className="border rounded-lg p-3 bg-muted/50">
                <Label htmlFor={`time-${order.id}`} className="text-xs font-medium flex items-center gap-2 mb-2">
                ...
                <div className="grid grid-cols-4 gap-1.5 mb-2">
                  {[5, 10, 15, 20].map((mins) => (
                    <Button
                      ...
                      onClick={() => handleTimeChange(order.id, mins)}
                    >
                      {mins}m
                    </Button>
                  ))}
                ...
                {/* Custom time input */}
                <form onSubmit={handleTimeSubmit} className="flex gap-2">
                  <Input
                    id={`time-${order.id}`}
                    type="number"
                    min="1"
                ...
                {/* Countdown timer or ready time */}
                {order.estimatedPickupTime && (
                  <div className="mt-2 flex items-center justify-between">
                    <p className="text-xs text-muted-foreground">
                      Ready at: {new Date(order.estimatedPickupTime).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                    </p>
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem onClick={handleGenerateSampleOrders}>
        ...
        <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-5 mb-8">
          ...
          {averagePrepTime !== null && (
            <Card>
              ...
              <CardContent>
                <div className="text-3xl font-bold">{averagePrepTime} min</div>
              </CardContent>
            </Card>
          )}
